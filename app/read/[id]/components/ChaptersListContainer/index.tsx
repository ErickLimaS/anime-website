...
function ChaptersListContainer({ mediaId, currChapterId, chaptersList }: ComponentTypes) {
            ...
            <motion.ol id={styles.list_container}>
                ...
                {chaptersList?.map((chapter, key) => (
                    ...
                    >
                        ...
                        <Link href={`/read/${mediaId}?source=mangadex&chapter=${chapter.chapterNumber}&q=${chapter.id}`}>
                        ...
                        <div className={styles.chapter_info_container}>
                            ...
                            <Link href={`/read/${mediaId}?source=mangadex&chapter=${chapter.chapterNumber}&q=${chapter.id}`}>
                                <h4>{chapter.pages == 0 ? `${chapter.title} (Not Available)` : chapter.title == chapter.chapterNumber ? `Chapter ${chapter.chapterNumber}` : chapter.title}</h4>
                            </Link>
