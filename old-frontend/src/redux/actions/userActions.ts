...
const MONGODB_USER_URL = "https://anime-website.up.railway.app/users"
...
// CHANGE SHOW ADULT CONTENT OPTION
export const changeAdultContentOption = () => async (dispatch: Dispatch<AnyAction>, getState: any) => {

    dispatch({
        type: CHANGE_SHOW_ADULT_CONTENT_REQUEST
    })
    ...
    try {
        ...
        dispatch({
            type: CHANGE_SHOW_ADULT_CONTENT_SUCCESS,
            payload: data
        })
        ...
        dispatch({
            type: CHANGE_SHOW_ADULT_CONTENT_FAIL,
            payload: error.response && error.response.data.message ? error.response.status : error.response.status
        })
...
export const darkModeSwitch = () => async (dispatch: Dispatch<AnyAction>, getState: any) => {
    ...
    try {
        ...
        console.log(error.response && error.response.data.message ? error.response.status : error.response.status)
